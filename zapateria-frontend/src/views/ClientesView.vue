<template>
  <div class="clientes-view">
    <div class="header">
      <h2>Clientes</h2>
      <button @click="openNew" class="btn-new">+ Nuevo Cliente</button>
    </div>
    ...
    <div class="table-wrap">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="c in items" :key="c.id_cliente">
            <td>{{ c.id_cliente }}</td>
            <td>{{ c.nombre }}</td>
            <td>{{ c.apellido }}</td>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-content">
        <header><h3>{{ editId ? 'Editar' : 'Nuevo' }} Cliente</h3></header>
...
const form = ref({ nombre: '', apellido: '' });
...
const openNew = () => { editId.value = null; form.value = { nombre: '', apellido: '' }; showModal.value = true; };
const edit = (c) => { editId.value = c.id_cliente; form.value = { nombre: c.nombre, apellido: c.apellido }; showModal.value = true; };
...
const save = async () => {
  if (!form.value.nombre || !form.value.apellido) { showToast('Nombre y apellido obligatorios', 'warn'); return; }
  try {
    if (editId.value) await updateCliente(editId.value, form.value);
    else await createCliente(form.value);
    await load();
    close();
...
const confirmDelete = (id) => { confirmId.value = id; confirmMsg.value = '¿Desea eliminar este cliente?'; showConfirm.value = true; };
const doDelete = async () => {
  try {
    await deleteCliente(confirmId.value);
    await load();
